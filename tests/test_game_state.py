...
def test_new_game_welcome_logs_gender_identity() -> None:
    gs = new_game_state("旅人", Profession.WARRIOR.value, gender="女")
...
def test_save_and_load_roundtrip(tmp_path: Path) -> None:
    gs = new_game_state("存档", Profession.MAGE.value, gender="自定义旅者")
    p = tmp_path / "t.json"
    gs.save(p)
    ...
    assert gs2.player.gender == "自定义旅者"
    assert gs2.current_scene_id == gs.current_scene_id
    assert gs2.game_state_domain_version == gs.game_state_domain_version
